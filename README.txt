...
这是南京大学软件学院推免考核 Coding Agent Harness 项目，基于 DeepSeek API 和 OpenAI-compatible tool_calls 协议实现，不使用 agent 框架或 SDK。项目按 Sprint 迭代开发，包含需求、Backlog、接口、架构、测试和实验报告。

主要功能：
1. 通过 CLI 接收编程任务。
2. 调用 DeepSeek 模型进行多轮推理。
3. 支持 list_files、read_file、write_file、run_command 四类工具。
4. 支持路径边界、危险命令拦截、超时、输出截断和错误阈值终止。
5. 生成 JSONL 日志，便于复盘。
